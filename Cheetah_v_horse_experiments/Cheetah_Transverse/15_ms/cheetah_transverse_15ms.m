%% Running test with final cheetah model
function X = cheetah_transverse_15ms(X0, k, zeta, counter)
% clear;
% clc;
...
P8.ConstraintList = P8.ConstraintList.AddConstraint("Total Time", (t1+t2+t3+t4+t5+t6+t7+t8), 0.25, 0.4); 
...
problem.Parameter.Value = 15; 
problem = BuildProblem(problem);
%%
% cheetahseed = load("cheetah_grounded_seed_N5.mat");
% X0 = cheetahseed.X;
...
save(append(num2str(k),'_k_cheetah_grounded_15ms.mat'),"Data")
