...
function obj = CreateCasadiModel(obj, model)
    import casadi.*

    NB = model.NB; % degrees of freedom
    if NB > 6
        NA = NB - 6; % number of actuated degrees of freedom
    ...
    contactdic = []; num = 1:NC; 
    for i = 1:NC
        grf{i} = SX.sym(append('grf_',model.gc.name{i}),3); % contact force variable
        foot{i} = posvel(1:3,i); % foot positions
        footFunc{i} = Function(append('footFunc_',model.gc.name{i}), {x}, {foot{i}},{'x'},{append('foot_',model.gc.name{i})}); 
        footspeed{i} = posvel(4:6,i); % foot positions
