function phase_data = ExtractMultiphaseSolution(solution_vector, phase_config)
...
for phase = 1:n_phases
    ...
    n_input = n_dof - 6;  % Torque input vector is 6 rows less
    
    % Calculate total variables per finite element
    % Order: time + pos + vel + acc + torque_input + contact_forces
    vars_per_element = 1 + n_dof + n_dof + n_dof + n_input + (n_contacts * 3);
    ...
    if n_contacts > 0
        contact_forces_raw = phase_matrix(row_idx:row_idx + (n_contacts * 3) - 1, :);
        % Reshape to organize contact forces: each contact gets 3 consecutive rows
        phase_data(phase).ground_reaction_forces = zeros(n_elements, 3, n_contacts);
        for contact = 1:n_contacts
            start_row = (contact - 1) * 3 + 1;
            end_row = contact * 3;
